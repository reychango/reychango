        ...
        <div className="mb-10 text-center">
          ...
          <div className="flex flex-wrap justify-center gap-2 mb-8">
            <button
              ...
              className={`px-4 py-2 rounded-full ${selectedAlbum === 'all'
            ...
            {albums.map(album => (
              <button
                ...
                className={`px-4 py-2 rounded-full ${selectedAlbum === album.name
                    ? 'bg-primary-500 text-white dark:bg-primary-600'
                    : 'bg-gray-200 text-gray-700 hover:bg-gray-300 dark:bg-gray-700 dark:text-gray-300 dark:hover:bg-gray-600'
                  }`}
              >
...
export async function getStaticProps() {
  try {
    ...
    // Calcular el conteo de fotos por álbum si no viene de Firestore
    const albumsWithCounts = allAlbums.map(album => {
      const count = photos.filter(p => p.album === album.name).length;
      return { ...album, count };
    });

    // Asegurarnos de que el álbum "Sin álbum" aparezca si hay fotos sin categoría
    const photosWithoutAlbum = photos.filter(p => !p.album || p.album === 'Sin álbum');
    if (photosWithoutAlbum.length > 0 && !albumsWithCounts.find(a => a.name === 'Sin álbum')) {
      albumsWithCounts.push({ name: 'Sin álbum', count: photosWithoutAlbum.length });
    }

    return {
      props: {
        photos: JSON.parse(JSON.stringify(photos)), // Forma más segura de serializar para Next.js
        albums: JSON.parse(JSON.stringify(albumsWithCounts)),
      },
      revalidate: 10,
    };
  } catch (error) {
