...
export default function Admin({ posts, photos }) {
  ...
  // Cargar configuración al estar autenticado
  useEffect(() => {
    if (isAuthenticated) {
      fetchSettings();
    }
        ...
        {activeTab === 'settings' && (
          <div className="max-w-2xl">
            ...
            <div className="bg-white dark:bg-gray-800 shadow-md rounded-lg p-6 border border-gray-100 dark:border-gray-700">
              <h3 className="text-lg font-medium mb-4 text-gray-900 dark:text-white border-b pb-2">Redes Sociales</h3>
              <p className="text-sm text-gray-500 dark:text-gray-400 mb-6">
                Configura los enlaces que aparecerán en el pie de página de tu blog.
              </p>

              <form onSubmit={handleSaveSettings} className="space-y-4">
                {Object.keys(socialLinks).map((key) => (
                  <div key={key}>
                    <label htmlFor={key} className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1 capitalize">
                      {key.replace(/([A-Z])/g, ' $1')}
                    </label>
                    <input
                      type="url"
                      ...
                      placeholder={`https://${key}.com/tuperfil`}
                    />
                  </div>
                ))}
